...
if not TOKEN:
    logger.error("TELEGRAM_BOT_TOKEN or BOT_TOKEN environment variable not set")
    print("Please provide TELEGRAM_BOT_TOKEN environment variable")
    exit(1)

if not DATABASE_URL:
    logger.error("DATABASE_URL environment variable not set")
    print("Please ensure PostgreSQL database is configured")
    exit(1)
...
class User(Base):
    ...
    likes_sent = Column(JSON, default=list)
